 ...
 * @author Promptnow11
 */
@BusinessProcess("FL_Save_Template")
public class FL_Save_Template extends BaseService{
    
    @JSON
    @Internal
    @Input(REQUEST)
    FL_Save_Template_RequestModel reqObject = null; 
        
    @JSON
    @Render(RESPONSE)
    FL_Save_Template_ResponseModel resObject = null;   
               
    @PreProcess
    public String preExecute(VulcanHashParameter vhp) throws VulcanException {
        jsonInput = getJSONMask(reqObject);
        resObject = new FL_Save_Template_ResponseModel();
    ...
    public String execute(VulcanHashParameter vhp) throws VulcanException {
        try{
            ...
            log.error(ex);
            Logger.getLogger(FL_Save_Template.class.getName()).log(Level.SEVERE, null, ex);
            resObject.setCaseError(reqObject, FL_Save_Template_ResponseModel.STATUS_ERROR, Message.get(reqObject.language,"GEN_ERR"));
        }finally {
...
class FL_Save_Template_RequestModel extends CommonRequestModel{
} 

class FL_Save_Template_ResponseModel extends CommonResponseModel{
    public FL_Save_Template_ResponseModel() {
        super();
    }
    
    public FL_Save_Template_ResponseModel(String status, String code, String msg) {
