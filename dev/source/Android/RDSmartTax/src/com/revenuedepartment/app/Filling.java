...
public class Filling extends Activity {
	...
	private class getPnd91 extends AsyncTask<String, Void, P_Filling> {
		...
		ConnectApi connApi = new ConnectApi(Filling.this);
		MRequest_Filling filling = new MRequest_Filling();
		String JSONObjSend;
		...
		protected void onPreExecute() {
			super.onPreExecute();
			dialog.show();
			filling.nid = mValue.getString(JSONElement.nid);
			filling.authenKey = mValue.getString(JSONElement.authenKey);
			filling.deviceID = new GetDeviceID(Filling.this).getDeviceID();
		}

		@Override
		protected P_Filling doInBackground(String... params) {
			filling.sessionID = PromptnowCommandData.JSESSIONID;
			JSONObjSend = new Gson().toJson(filling);
			P_Filling rs = connApi.requestGetPnd91(JSONObjSend);
			return rs;
		}
