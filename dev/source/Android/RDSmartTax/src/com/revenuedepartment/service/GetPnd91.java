				...
				for (int i = 0; i < formDataArr.length(); i++) {
					...
					if (ojbFielsArr != null && ojbFielsArr.length() > 0) {
						...
						for (int j = 0; j < ojbFielsArr.length(); j++) {
							...
							fields.setKeyboardType(getJsonString(objFields, JSONElement.placeholder));
			...
			for (int i = 0; i < obj.length(); i++) {
				try {
					...
					if (hasElement(objSummary, JSONElement.maximum)) {
						JSONArray objMaximum = objSummary.getJSONArray(JSONElement.maximum);
						mSummaryArr[i].setMaximum(getObjConSummary(objMaximum));
					}
					return mSummaryArr;
				} catch (JSONException e) {
					e.printStackTrace();
					...
					if (objFieldsArr != null && objFieldsArr.length() > 0) {
						...
						for (int j = 0; j < objFieldsArr.length(); j++) {
							...
							if (hasElement(objFields, JSONElement.conditions)) {
								JSONArray objConArr = objFields.getJSONArray(JSONElement.conditions);
								if (objConArr != null && objConArr.length() > 0) {
									M_Conditions[] conArr = new M_Conditions[objConArr.length()];
									for (int k = 0; k < mDepenOnFieldsArr.length; k++) {
										JSONObject objCon = objConArr.getJSONObject(k);
										conArr[k] = new M_Conditions();
										conArr[k].setIdentify(getJsonString(objFields, JSONElement.identify));
										conArr[k].setOperate(getJsonString(objFields, JSONElement.operate));
