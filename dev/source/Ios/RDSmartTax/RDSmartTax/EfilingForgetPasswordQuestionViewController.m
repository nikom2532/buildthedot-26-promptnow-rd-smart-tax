...
@synthesize enConfirmQuestionService;
...

{
    ...
    if(tag == 0)
    {
       ...
       [dropDownPickerView selectRow:selectedQuestion inComponent:0 animated:NO];
    }
...
- (void) setLanguage {
    ...
    [self.txtQuestion setPlaceholder:[Util loadPlaceholder:@"question"]];
    [self.txtAnswer setPlaceholder:[Util loadPlaceholder:@"answer"]];
}

- (void) setFontStyle {
    
    [self.txtQuestion setFont:[FontUtil fontWithFontSize:eFontSizeNormal Style:eFontStyleBold]];
    [self.txtAnswer setFont:[FontUtil fontWithFontSize:eFontSizeNormal Style:eFontStyleBold]];
    self.txtQuestion.textAlignment = UITextAlignmentRight;
    self.txtAnswer.textAlignment = UITextAlignmentRight;
    [self.labelAnswer setFont:[FontUtil fontWithFontSize:eFontSizeSmall Style:eFontStyleNormal]];
...
- (BOOL) validateField {
    ...
    if(![strSelectedQuestion isEqualToString:@""]){
         strSelectedQuestion = [questionId objectAtIndex:selectedQuestion];
         NSLog(@"selectedQuestion : %@", strSelectedQuestion);
    }
    ...
    if([Util validateEmptyFieldWithString:strSelectedQuestion]){
        ...
        NSLog(@"answer empty");
        [self alertResultWithTitle:@"" detail:[Util loadErrorMessageWithValidateField:@"answerEmpty"] yes:[Util stringWithScreenName:@"Common" labelName:@"Yes"]];
        txtActiveEditing = self.txtAnswer;
        return NO;
    }else{
        return YES;
    }
...
- (IBAction)onForgotButtonClicked:(id)sender {
    ...
    NSLog(@"thai nation>>%@",self.pThaiNation);
...
- (void) callConfirmQuestionService{
    ...
    [enConfirmQuestionService requestENConfirmQuestion:self.pNid
                                            ...
                                            questionId:strSelectedQuestion];
...
- (void) responseENConfirmQuestionService:(NSDictionary *)data{
    ...
    if([[dic objectForKey:@"responseCode"]isEqualToString:@"0"]){
        ...
        if([[dic objectForKey:@"responseStatus"] isEqualToString:@"ERROR"]) {
            [self alertResultWithTitle:[Util stringWithScreenName:@"Common" labelName:@"Sorry"] detail:[dic objectForKey:@"responseError"] yes:[Util stringWithScreenName:@"Common" labelName:@"Yes"]];
        }
    }else{
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 0) {
        if([response isEqualToString:@"validate"]){
