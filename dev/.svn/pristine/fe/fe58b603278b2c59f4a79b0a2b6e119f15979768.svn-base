 ...
 * @author Promptnow11
 */
@BusinessProcess("FL_Delete_Form")
public class FL_Delete_Form extends BaseService{
    
    @JSON
    @Internal
    @Input(REQUEST)
    FL_Delete_Form_RequestModel reqObject = null; 
        
    @JSON
    @Render(RESPONSE)
    FL_Delete_Form_ResponseModel resObject = null;   
               
    @PreProcess
    public String preExecute(VulcanHashParameter vhp) throws VulcanException {
        jsonInput = getJSONMask(reqObject);
        resObject = new FL_Delete_Form_ResponseModel();
    ...
    public String execute(VulcanHashParameter vhp) throws VulcanException {
        try{
            ...
            DeleteFormOutputModel wsRes = ws.deleteForm(reqObject.nid, reqObject.authenKey, reqObject.language, reqObject.sysRefNo);
            ...
            log.error(ex);
            Logger.getLogger(FL_Delete_Form.class.getName()).log(Level.SEVERE, null, ex);
            resObject.setCaseError(reqObject, FL_Delete_Form_ResponseModel.STATUS_ERROR, Message.get(reqObject.language,"GEN_ERR"));
        }finally {
...
class FL_Delete_Form_RequestModel extends CommonRequestModel{
    public String sysRefNo;
} 

class FL_Delete_Form_ResponseModel extends CommonResponseModel{
        
    public FL_Delete_Form_ResponseModel() {
        super();
    }
    
    public FL_Delete_Form_ResponseModel(String status, String code, String msg) {
