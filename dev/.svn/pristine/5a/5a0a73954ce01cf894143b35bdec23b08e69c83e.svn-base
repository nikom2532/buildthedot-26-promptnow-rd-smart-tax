...
public class ResetPasswordNoHaveEmailRequestStep2 extends Activity implements OnClickListener {
	...
	Button mbuttonForgotPasswordNoHaveEmailStep2Submit;
	...
	public void setView() {
		...
		mbuttonForgotPasswordNoHaveEmailStep2Submit = (Button) findViewById(R.id.mbuttonForgotPasswordNoHaveEmailStep2Submit);
		...
		protected void onPostExecute(M_ResetPasswordQuestionChangeOnlyPassword result) {
			...
			if (result == null) {
				...
				if(result.getResponseStatus().equals("OK")){
					finish();
					Intent intent = new Intent(ResetPasswordNoHaveEmailRequestStep2.this, Login_E_Filing.class);
	...
	public void onClick(View v) {
		...
		if (mbuttonForgotPasswordNoHaveEmailStep2Submit.equals(v)) {
			...
			if (mEditTextForgetPasswordNoHaveEmailStep2Password.getText().toString().equalsIgnoreCase("")) {
				Toast.makeText(ResetPasswordNoHaveEmailRequestStep2.this, "โปรดใส่ รหัสผ่าน", 
						Toast.LENGTH_SHORT).show();
			}
			else if (mEditTextForgetPasswordNoHaveEmailStep2PasswordAgain.getText().toString().equalsIgnoreCase("")) {
				Toast.makeText(ResetPasswordNoHaveEmailRequestStep2.this, "โปรดใส่ รหัสผ่านตัวที่ 2 อีกครั้ง", 
						Toast.LENGTH_SHORT).show();
			}
			else if (!mEditTextForgetPasswordNoHaveEmailStep2Password.getText().toString().equals(mEditTextForgetPasswordNoHaveEmailStep2PasswordAgain.getText().toString())) {
				Toast.makeText(ResetPasswordNoHaveEmailRequestStep2.this, "รหัสผ่าน2ตัว ไม่เหมือนกัน โปรดใส่อีกครั้ง", 
						Toast.LENGTH_SHORT).show();
			}
			else{
