...
@interface EfilingChangeOnlyPasswordViewController (){
    ENChangeOnlyPasswordService *enChangeOnlyPasswordService;
...
- (void) setLanguage {
    ...
    [self.txtNewPassword setPlaceholder:[Util stringWithScreenName:@"Common" labelName:@"NewPassword"]];
    [self.txtConfirmNewPassword setPlaceholder:[Util stringWithScreenName:@"Common" labelName:@"ConfirmNewPassword"]];
    
}

- (void) setFontStyle {
    
    [self.labelNewPassword setFont:[FontUtil fontWithFontSize:eFontSizeNormal Style:eFontStyleBold]];
    [self.labelConfirmPassword setFont:[FontUtil fontWithFontSize:eFontSizeNormal Style:eFontStyleBold]];
...
- (BOOL) validateField {
    if([Util validatePasswordIsEqualConfirmPassword:self.txtNewPassword.text confirmPassword:self.txtConfirmNewPassword.text]){
        NSLog(@"Password field not match ");
        [Util alertUtilWithTitle:[Util stringWithScreenName:@"Common" labelName:@"Sorry"] msg:@""];
        return NO;
    }else if([Util validatePasswordIsEmpty:self.txtNewPassword.text confirmPassword:self.txtConfirmNewPassword.text]){
        NSLog(@"Password field must be 8 character ");
        [Util alertUtilWithTitle:[Util stringWithScreenName:@"Common" labelName:@"Sorry"] msg:@""];
        return NO;
    }else if([Util validatePasswordIsExceptWord:self.txtNewPassword.text confirmPassword:self.txtConfirmNewPassword.text]){
...
- (void) callChangeOnlyPassword{
    ...
    [enChangeOnlyPasswordService requestENChangeOnlyPasswordService:nid password:self.txtNewPassword.text version:version ];
