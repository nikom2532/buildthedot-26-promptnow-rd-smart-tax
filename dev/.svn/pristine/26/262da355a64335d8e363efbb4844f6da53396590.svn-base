 ...
 * @author Promptnow11
 */
@BusinessProcess("FG_Reset_Password_Confirm")
public class FG_Reset_Password_Confirm extends BaseService{
    
    @JSON
    @Internal
    @Input(REQUEST)
    FG_Reset_Password_Confirm_RequestModel reqObject = null; 
        
    @JSON
    @Render(RESPONSE)
    FG_Reset_Password_Confirm_ResponseModel resObject = null;   
               
    @PreProcess
    public String preExecute(VulcanHashParameter vhp) throws VulcanException {
        jsonInput = getJSONMask(reqObject);
        resObject = new FG_Reset_Password_Confirm_ResponseModel();
    ...
    public String execute(VulcanHashParameter vhp) throws VulcanException {
        try{
            ...
            ResetPasswordConfirmOutputModel wsRes = ws.resetPasswordConfirm(reqObject.nid, reqObject.language, reqObject.authenKey, reqObject.email);
            ...
            log.error(ex);
            Logger.getLogger(FG_Reset_Password_Confirm.class.getName()).log(Level.SEVERE, null, ex);
            resObject.setCaseError(reqObject, FG_Reset_Password_Confirm_ResponseModel.STATUS_ERROR, Message.get(reqObject.language,"GEN_ERR"));
        }finally {
...
class FG_Reset_Password_Confirm_RequestModel extends CommonRequestModel{
    public String email;
} 

class FG_Reset_Password_Confirm_ResponseModel extends CommonResponseModel{
    
    public FG_Reset_Password_Confirm_ResponseModel() {
        super();
    }
    
    public FG_Reset_Password_Confirm_ResponseModel(String status, String code, String msg) {
