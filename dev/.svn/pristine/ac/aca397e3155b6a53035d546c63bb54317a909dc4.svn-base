 ...
 * @author Promptnow11
 */
@BusinessProcess("FG_Change_Password")
public class FG_Change_Password extends BaseService{
    
    @JSON
    @Internal
    @Input(REQUEST)
    FG_Change_Password_RequestModel reqObject = null; 
        
    @JSON
    @Render(RESPONSE)
    FG_Change_Password_ResponseModel resObject = null;   
               
    @PreProcess
    public String preExecute(VulcanHashParameter vhp) throws VulcanException {
        jsonInput = getJSONMask(reqObject);
        resObject = new FG_Change_Password_ResponseModel();
    ...
    public String execute(VulcanHashParameter vhp) throws VulcanException {
        try{
            ...
            ChangePasswordOutputModel wsRes = ws.changePassword(reqObject.userId, reqObject.language, reqObject.authenKey, reqObject.oldPassword, reqObject.newPassword);
            ...
            log.error(ex);
            Logger.getLogger(FG_Change_Password.class.getName()).log(Level.SEVERE, null, ex);
            resObject.setCaseError(reqObject, FG_Change_Password_ResponseModel.STATUS_ERROR, Message.get(reqObject.language,"GEN_ERR"));
        }finally {
...
class FG_Change_Password_RequestModel extends CommonRequestModel{
...
class FG_Change_Password_ResponseModel extends CommonResponseModel{
    
    public FG_Change_Password_ResponseModel() {
        super();
    }
    
    public FG_Change_Password_ResponseModel(String status, String code, String msg) {
