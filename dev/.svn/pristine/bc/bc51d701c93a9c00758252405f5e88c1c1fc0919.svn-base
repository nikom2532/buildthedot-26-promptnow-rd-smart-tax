 ...
 * @author Promptnow11
 */
@BusinessProcess("FG_Confirm_Request_Password")
public class FG_Confirm_Request_Password extends BaseService{
    
    @JSON
    @Internal
    @Input(REQUEST)
    FG_Confirm_Request_Password_RequestModel reqObject = null; 
        
    @JSON
    @Render(RESPONSE)
    FG_Confirm_Request_Password_ResponseModel resObject = null;   
               
    @PreProcess
    public String preExecute(VulcanHashParameter vhp) throws VulcanException {
        jsonInput = getJSONMask(reqObject);
        resObject = new FG_Confirm_Request_Password_ResponseModel();
    ...
    public String execute(VulcanHashParameter vhp) throws VulcanException {
        try{
            ...
            ConfirmRequestPasswordOutputModel wsRes = ws.confirmRequestPassword(reqObject.nid, reqObject.language, reqObject.authenKey, reqObject.name, reqObject.surName, reqObject.birthDate, reqObject.fatherName, reqObject.motherName, reqObject.passportNo, reqObject.countryCode, reqObject.middleName);
            ...
            log.error(ex);
            Logger.getLogger(FG_Confirm_Request_Password.class.getName()).log(Level.SEVERE, null, ex);
            resObject.setCaseError(reqObject, FG_Confirm_Request_Password_ResponseModel.STATUS_ERROR, Message.get(reqObject.language,"GEN_ERR"));
        }finally {
...
class FG_Confirm_Request_Password_RequestModel extends CommonRequestModel{
    public String name;
...
class FG_Confirm_Request_Password_ResponseModel extends CommonResponseModel{
    
    public FG_Confirm_Request_Password_ResponseModel() {
        super();
    }
    
    public FG_Confirm_Request_Password_ResponseModel(String status, String code, String msg) {
