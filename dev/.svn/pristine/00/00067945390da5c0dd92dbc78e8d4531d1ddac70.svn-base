...
//                nil];
    ...
    if (![dateStr isEqualToString:@""] || [dateStr isEqualToString:@"(null)"]) {
        /*
...
//            cell.textLabel.textColor = [UIColor blackColor];
    ...
    UIImageView *imv = [[UIImageView alloc]initWithFrame:CGRectMake(55, 50-img.size.height, img.size.width, img.size.height)];
    imv.image = img;
    [cell addSubview:imv];
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 0) {
        ...
        if ([Util isInternetConnect]) {
            ...
            [enLogoutService requestENLogoutServiceWithNid:[ShareUserDetail retrieveDataWithStringKey:@"nid"]];
...
{
    JLLog(@"%@",data);
    if ([data objectForKey:@"responseData"]==nil) {
        EflingTermAndConditionViewController *tax = [[EflingTermAndConditionViewController alloc]initWithTypeOfPnd:statusPay];
        [self.navigationController pushViewController:tax animated:YES];
